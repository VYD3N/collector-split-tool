...
describe('CollectorRanking', () => {
    ...
    beforeEach(() => {
        ...
        jest.clearAllMocks();
        (api.getCollectors as jest.Mock).mockResolvedValue({ collectors: mockCollectors });
    });
    ...
    describe('Filter Controls', () => {
        ...
        it('filters by minimum NFTs', async () => {
            ...
            // Use getByRole to find the spinbutton within the NumberInput
            const nftInput = within(screen.getByTestId('min-nfts-input')).getByRole('spinbutton');
            fireEvent.change(nftInput, { target: { value: '10' } });
    ...
    describe('Sorting Behavior', () => {
        ...
        it('sorts addresses correctly', async () => {
            ...
            // Wait for first row to be correct
            await waitFor(() => {
                const firstRow = screen.getByTestId('collector-row-0');
                expect(firstRow).toHaveTextContent('tz1test1');
            });

            // Check remaining rows
            const secondRow = screen.getByTestId('collector-row-1');
            const thirdRow = screen.getByTestId('collector-row-2');
            expect(secondRow).toHaveTextContent('tz1test2');
            expect(thirdRow).toHaveTextContent('tz1test3');
        });
    });
